...
import { syncDataset } from "./sync-dataset.mjs";
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--dashboard-root") {
      result.dashboardRoot = argv[index + 1] ? path.resolve(argv[index + 1]) : defaultDashboardRoot;
      index += 1;
      continue;
    }
...
async function main() {
  ...
  writeLine(`Watching canonical dashboard notes under ${path.relative(defaultDashboardRoot, path.resolve(dashboardRoot, "..")).replaceAll("\\", "/")}`);
  ...
  try {
    ...
    const scheduleRefresh = () => {
      void runRefresh("file change").catch((error) => {
        writeError(`Dashboard note sync failed: ${error instanceof Error ? error.stack ?? error.message : String(error)}`);
      });
    };
    ...
    writeError(`Initial dashboard note sync failed: ${error instanceof Error ? error.stack ?? error.message : String(error)}`);
    process.exitCode = 1;
    await stop();
  }
}
