...
function waitForText(stream, expectedText, timeoutMs = 15000) {
...
test("watch-notes-and-sync reruns the pipeline when a canonical note changes", async () => {
  ...
  try {
    ...
    await waitForText(watcher.stdout, "Watching canonical dashboard notes", 15000);
    await waitForText(watcher.stdout, "Dashboard notes refreshed", 15000);
